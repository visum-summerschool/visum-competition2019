...
PREDICTIONS_FILE = '/home/jtrp/VISUM_baseline/predictions.csv'
LABELS_DIR = ''

labels_file = os.path.join(LABELS_DIR, )


# Para calcular mAP entre csv de previsões e csv de ground-truths
# CÓDIGO PREPARADO PARA APENAS UM OBJETO POR IMAGEM
...
def compute_mAP(preds_file, labels_file):
    # reading csv files
    data = pd.read_csv(labels_file, header=None, names=['image_name', 'xmin', 'ymin', 'xmax', 'ymax', 'class'])
    preds = pd.read_csv(preds_file, header=None, names=['image_name', 'xmin', 'ymin', 'xmax', 'ymax', 'class'])

    # merging predicted coordinates with the original dataframe
